  ...
  return new Promise(async (resolve, reject) => {
    try {
      let users = await db.User.findAll({
        ...
        where: { 
          roleId: {
            [Op.or]: ["R2", "R4"], // Lọc roleId là R2 hoặc R4
          },
        },
              ...
              include: [
                {
                  model: db.Specialty,
                  as: "specialtyData",
                  attributes: ["name"], // Chỉ lấy tên của Specialty
                },
              ],
            },
          ],
...
let getListAppointmentsForUser = (userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!userId) {
        // Reject if userId is missing
        return reject({
          errCode: 1,
      ...
      let data = await db.Booking.findAll({
        where: {  statusId: "S2", patientId: userId }, // Use patientId to match the user
        include: [
          {
            model: db.User,
            as: "patientData", // Alias to link with User
            attributes: [
              "email",
              "firstName",
              "lastName",
            ...
            include: [
              {
                model: db.Allcode,
                as: "genderData", // Include gender data from Allcode
                attributes: ["valueEn", "valueVi"],
              },
            ],
          },
          {
            model: db.Allcode,
            as: "timeTypeDataPatient", // Include time type data from Allcode
            attributes: ["valueEn", "valueVi"],
          },
          {
            model: db.Doctor_Infor,
        ...
        raw: false, // Sequelize will return instances instead of raw data
        nest: true,  // Nest related model data
      });

      // Resolve with the result, or an empty array if no data
      resolve({
        errCode: 0,
        data: data || [], // Return an empty array if no bookings found
      });
    } catch (e) {
...
module.exports = {
  ...
  getListAppointmentsForUser:getListAppointmentsForUser
};
