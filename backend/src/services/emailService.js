...
let sendSimpleEmail = async (dataSend) => {
  ...
  let info = await transporter.sendMail({
    from: '"Nguyễn Văn sơn" <son@gmail.com>', // sender address
    to: dataSend.receiverEmail, // list of receivers
    subject: "THÔNG TIN ĐẶT LỊCH DỊCH VỤ", // Subject line
...
<p>Bạn nhận được email này vì đã đặt lịch chăm sóc thú cưng  online trên mywebsite</p>
<p>Thông tin đặt lịch đặt lịch chăm sóc thú cưng :</p>
<div><b>Thời gian: ${dataSend.time}</b></div>
<div><b>Nhân viên: ${dataSend.doctorName}</b></div>

<p>Nếu các thông tin trên là đúng sự thật, vui lòng click vào đường link bên dưới để hoàn tất thủ tục đặt lịch khám bệnh.</p>
<div><a href=${dataSend.redirectLink} style="
    ...
    border-radius: 5px;
    font-weight: bold; target="_blank">Đặt lịch</a></div>
...
<h3><b>Xin chào ${dataSend.patientName}!</b></h3>
<p>Bạn nhận được email này vì đã đặt lịch chăm sóc thú cưng online trên mywebsite</p>
<p>Thông tin nội dung cần chú ý trong file đính kèm.</p>
<div>Xin chân thành cảm ơn!</div>
`;
  }
  if (dataSend.language === "en") {
...
let sendAttachment = async (dataSend) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // send mail with defined transport object
      let info = await transporter.sendMail({
        from: '"Nguyễn Văn sơn" <son@gmail.com>', // sender address/ sender address
