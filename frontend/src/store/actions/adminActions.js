...
export const fetchUsersStart = (userId) => {
  return async (dispatch, getState) => {
    try {
...
export const saveDetailDoctor1 = (data) => {
  return async (dispatch, getState) => {
      try {
...
export const saveDetailDoctorAction = (data) => {
  return async (dispatch, getState) => {
    try {
      ...
      if (res && res.errCode === 0) {
        toast.success("Lưu thông tin chi tiết thành công!");
        dispatch({
          type: actionTypes.SAVE_DETAIL_DOCTOR_SUCCESS,
        });
      } else {
        toast.error("Lưu thông tin chi tiết không thành công!");
        dispatch({
          type: actionTypes.SAVE_DETAIL_DOCTOR_FAILED,
        });
      }
